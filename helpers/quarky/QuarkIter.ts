...
export class QuarkIter<T> implements Iterable<T> {
	...
	filter<U extends T>(
		...
		return new QuarkIter(new FilterIterator(this.iterator, predicate));
	}

	concat(other: QuarkIter<T>): QuarkIter<T> {
		return new QuarkIter(new ConcatIterator(this.iterator, other.iterator));
	}

	map<U>(fn: (item: T, index: number) => U): QuarkIter<U> {
		return new QuarkIter(new MapIterator(this.iterator, fn));
	}
	...
	flatten(): QuarkIter<Flattened<T>> {
		return new QuarkIter(new FlattenIterator(this.iterator));
	}

	zip(other: QuarkIter<T>): QuarkIter<[T, T]> {
		return new QuarkIter(new ZipIterator(this.iterator, other.iterator));
	}
