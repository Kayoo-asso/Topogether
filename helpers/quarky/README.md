...
Derivations essentially behave like quarks, except that they are read-only. The `Signal<T>` interface is common to both and means that they are functions `() => T`. Quarks just have an additional setter. From now on, we'll use the term _signals_ to refer to both quarks and derivations.
...
```ts
const nonMemoised = watchDependencies(
  (props: Props) => {
    // ...
  },
  { memo: false }
...
const Parent = watchDependencies(({ selected }: Props) => {
  return <>{selected() && <ShowSelected selected={selected()!} />}</>;
});
```
