...
export class InMemorySync implements SyncService {
    ...
    private like_rpc<K extends LikeKey>(key: K): PromiseLike<boolean> {
        ...
        this[key] = new Set();
        return supabaseClient.rpc(key, { _ids: Array.from(ids) })
            .then(res => {
                if (res.error) {
...
type UpdateKey = {
    [K in keyof InMemorySync]: InMemorySync[K] extends Map<UUID, any> ? K extends `updated${infer _}` ? K : never : never
}[keyof InMemorySync];

type UpdateValue<K extends UpdateKey> = MapValue<InMemorySync[K]>;

type DeleteKey = {
    [K in keyof InMemorySync]: InMemorySync[K] extends Set<UUID> ? K extends `deleted${infer _}` ? K : never : never
}[keyof InMemorySync];
