import {
	...
	SelectedBoulder,
	SelectedItem,
} from "components/store/selectStore";
	...
	if (img) {
		if (currentTool === "PARKING") {
			...
			if (selectedItem?.type === "boulder") {
				...
				select.image(img);
				select.track(
					createTrack(selectedItem.value(), session.id),
					selectedItem.value
				);
			} else {
				const newBoulderQuark = createBoulder(topo, coords, img);
				select.track(
					createTrack(newBoulderQuark(), session.id),
					newBoulderQuark
				);
			}
		}
