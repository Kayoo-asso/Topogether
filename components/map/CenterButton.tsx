...
export const CenterButton: React.FC<CenterButtonProps> = watchDependencies((props: CenterButtonProps) => {
    ...
    const extent = props.topo ? getTopoExtent(props.topo(), DEFAULT_EXTENT_BUFFER) : null;
    ...
    return (
        <RoundButton onClick={handleClick}>
            <CenterIcon 
                className={`h-7 w-7 ${activated ? 'fill-main stroke-main' : 'fill-grey-light stroke-grey-light'}`} 
            />
        </RoundButton>
    )
});
