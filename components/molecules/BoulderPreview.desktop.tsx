...
>(({ displayAddButton = false, ...props }: BoulderPreviewDesktopProps, parentRef) => {
	...
	return (
		<>
								...
								? useCallback(
										(id) => {
											const tracksOnTheImage = boulder.tracks
												.quarks()
												.filter((t) => !!t().lines?.find((l) => l.imageId === id))
												.toArray();
			...
			>
				Tous les passages dessinés sur cette image seront supprimés. Etes-vous sûr de vouloir
				continuer ?
			</ModalDeleteImage>
		</>
	);
