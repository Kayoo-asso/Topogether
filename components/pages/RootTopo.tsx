...
export const RootTopo: React.FC<RootTopoProps> = watchDependencies((props: RootTopoProps) => {
	...
	const toggleSectorSelect = useCallback(
		(sectorQuark: Quark<Sector>) => {
			if (selectedSector()?.id === sectorQuark().id) selectedSector.select(undefined);
	...
	const toggleBoulderSelect = useQuarkyCallback(
		(boulderQuark: Quark<Boulder>) => {
			...
			if (selectedBoulder()?.id === boulder.id) selectedBoulder.select(undefined);
			else {
	...
	const toggleParkingSelect = useQuarkyCallback(
		(parkingQuark: Quark<Parking>) => {
			...
			selectedTrack.select(undefined);
			selectedWaypoint.select(undefined);
			if (selectedParking()?.id === parkingQuark().id) selectedParking.select(undefined);
	...
	const toggleWaypointSelect = useQuarkyCallback(
		(waypointQuark: Quark<Waypoint>) => {
			...
			selectedTrack.select(undefined);
			selectedParking.select(undefined);
			if (selectedWaypoint()?.id === waypointQuark().id) selectedWaypoint.select(undefined);
	...
	useLazyQuarkyEffect(
		([boulder]) => {
			if (boulder)
				...
				router.push({ pathname: window.location.href.split("?")[0] }, undefined, { shallow: true });
		},
		[selectedBoulder]
	);
		...
		// TODO : add topo.creator.mail as the first option of the OR (after mailto)
			...
			menuOptions.push({
				...
				action: () =>
					window.open(
						"mailto:" +
							(topo.managers.toArray()[0]?.contactMail || "contact@kayoo-asso.fr") +
