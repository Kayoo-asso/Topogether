				...
				<div className="flex flex-col h-[60%]">
					...
					<div className={`grid grid-cols-8 p-5 items-center ${full ? "" : " mt-3"}`}>
						...
						<div className="flex flex-row gap-5 justify-end col-span-2">
							{selectedTrack && boulder.tracks.length > 1 && (
								<button onClick={() => setDisplayPhantomTracks(!displayPhantomTracks)}>
									<ManyTracks
										className={
											"w-6 h-6 " + (displayPhantomTracks ? "stroke-main" : "stroke-grey-medium")
										}
									/>
								</button>
							)}
					...
					{full && (
						<div className="flex flex-row gap-8 w-full px-5 ktext-label font-bold my-2">
							<span
								className={`${officialTrackTab ? "text-main" : "text-grey-medium"}`}
