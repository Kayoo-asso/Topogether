	...
	if (!props.access) {
		return (
			<div className="w-full pt-[10%]" onClick={(e) => e.stopPropagation()}>
				<Button content="Ajouter une marche d'approche" fullWidth onClick={props.onCreateAccess} />
			</div>
		);
	} else {
		...
		return (
			<>
				...
				<Button content="Supprimer" fullWidth onClick={() => props.onDeleteAccess(props.access)} />
			</>
		);
	}
});
